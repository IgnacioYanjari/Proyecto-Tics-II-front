...
import CreateMachineProduct from "components/modals/create/MachineProduct";
  ...
  save(form, key) {
    form.validateFields(async (error, row) => {
      ...
      if (index > -1) {
        ...
        let res = await this.productService.updateMachine({name, price}, id);
...
class MachineComponent extends Component {
  ...
  loadMachines() {
    this.productService.getMachines().then(res => {
      if (res.status === "success") {
      ...
      <div className="table-responsive">
        {machinesLen !== 0 ? (
          <div>
            ...
            <CreateMachineProduct refreshTable={this.loadMachines} />
            <EditableTable
