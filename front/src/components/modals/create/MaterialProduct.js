...
class MaterialProduct extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
  ...
  componentDidMount() {
    this.typeService.getMaterials().then(res => {
      this.setState({
        types: res.data,
        type: res.data[0]
      });
  ...
  closeModal() {
    document.getElementById("modalButtonMaterialProduct").click();
    this.setState({
      name: "",
      price: 10,
      brand: "",
  ...
  render() {
    ...
    return (
      <div>
        <button
          type="button"
          id="modalButtonMaterialProduct"
          className="btn btn-primary"
          data-toggle="modal"
          data-target="#modalMaterialProduct"
        >
          Crear
        </button>
